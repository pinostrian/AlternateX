...
local requestsDisabled = true --getgenv and getgenv().DISABLE_AlternateX_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.68"
local AlternateXFolder = "AlternateX"
local ConfigurationFolder = AlternateXFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		AlternateXOpen = {Type = 'bind', Value = 'K', Name = 'AlternateX Keybind'},
		-- buildwarnings
		-- AlternateXprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.AlternateXOpen"] = "J"
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("AlternateX | Function failed with error: ", result)
			return false
		else
			return result
		end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if callSafely(isfolder, AlternateXFolder) then
				if callSafely(isfile, AlternateXFolder..'/settings'..ConfigurationExtension) then
					file = callSafely(readfile, AlternateXFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"AlternateXOpen":{"Value":"K","Type":"bind","Name":"AlternateX Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"AlternateX Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('AlternateX had an issue accessing configuration saving capability.')
		end
	end
end
...
if not requestsDisabled then
	...
	if cachedSettings and (#cachedSettings == 0 or (cachedSettings.System and cachedSettings.System.usageAnalytics and cachedSettings.System.usageAnalytics.Value)) then
		sendReport("execution", "AlternateX")
...
local AlternateX = useStudio and script.Parent:FindFirstChild('AlternateX') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local AlternateXDestroyed = false -- True when AlternateXLibrary:Destroy() is called

repeat
	if AlternateX:FindFirstChild('Build') and AlternateX.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('AlternateX | Build Mismatch')
		print('AlternateX may encounter issues as you are running an incompatible interface version ('.. ((AlternateX:FindFirstChild('Build') and AlternateX.Build.Value) or 'No Build') ..').\n\nThis version of AlternateX is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, AlternateX = AlternateX, useStudio and script.Parent:FindFirstChild('AlternateX') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

AlternateX.Enabled = false

if gethui then
	AlternateX.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(AlternateX)
	AlternateX.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	AlternateX.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == AlternateX.Name and Interface ~= AlternateX then
			Interface.Enabled = false
			Interface.Name = "AlternateX-Old"
		end
	end
elseif not useStudio then
...
if AlternateX.AbsoluteSize.X < minSize.X and AlternateX.AbsoluteSize.Y < minSize.Y then
...
local Main = AlternateX.Main
local MPrompt = AlternateX:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = AlternateX:FindFirstChild('Drag')
...
AlternateX.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/AlternateX/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = AlternateX.Notifications
local keybindConnections = {} -- For storing keybind connections to disconnect when AlternateX is destroyed

local SelectedTheme = AlternateXLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = AlternateXLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	AlternateX.Main.BackgroundColor3 = SelectedTheme.Background
	AlternateX.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	AlternateX.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	AlternateX.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	AlternateX.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	AlternateX.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	AlternateX.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		AlternateX.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		AlternateX.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(AlternateX:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("AlternateX | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("AlternateX | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('AlternateX had an issue decoding the configuration file, please try delete the file and reopen AlternateX.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(AlternateXLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("AlternateX | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--AlternateXLibrary:Notify({Title = "AlternateX Flags", Content = "AlternateX was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(AlternateXLibrary.Flags) do
...
function AlternateXLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('AlternateX | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			AlternateXLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			AlternateXLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "AlternateXOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		callSafely(writefile, AlternateXFolder..'/settings'..ConfigurationExtension, encoded)
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('AlternateX Settings', 0, true)

	if TabList['AlternateX Settings'] then
		TabList['AlternateX Settings'].LayoutOrder = 1000
	end

	if Elements['AlternateX Settings'] then
		Elements['AlternateX Settings'].LayoutOrder = 1000
	end
...
function AlternateXLibrary:CreateWindow(Settings)
	if AlternateX:FindFirstChild('Loading') then
		if getgenv and not getgenv().AlternateXCached then
			AlternateX.Enabled = true
			AlternateX.Loading.Visible = true

			task.wait(1.4)
			AlternateX.Loading.Visible = false
		end
	end

	if getgenv then getgenv().AlternateXCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				AlternateXLibrary:Notify({Title = 'Build Mismatch', Content = 'AlternateX may encounter issues as you are running an incompatible interface version ('.. ((AlternateX:FindFirstChild('Build') and AlternateX.Build.Value) or 'No Build') ..').\n\nThis version of AlternateX is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "AlternateX"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "AlternateX Interface Suite" then
		LoadingFrame.Version.Text = "AlternateX UI"
	end
	...
	if not Settings.DisableAlternateXPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				AlternateXLibrary:Notify({
					Title = "AlternateX Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		ensureFolder(AlternateXFolder.."/Discord Invites")

		if callSafely(isfile, AlternateXFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				callSafely(writefile, AlternateXFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"AlternateX RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		ensureFolder(AlternateXFolder.."/Key System")
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("AlternateX | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AlternateX specific development.')
				end
			end
		end
		...
		if callSafely(isfile, AlternateXFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				local savedKeys = callSafely(readfile, AlternateXFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						callSafely(writefile, AlternateXFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						AlternateXLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called AlternateXLibrary:Destroy()
				if AlternateXDestroyed then
					return
				end
				if not Success then
					...
					print("AlternateX | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AlternateX specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= AlternateXLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					AlternateXLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			AlternateX.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("AlternateX | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AlternateX specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					AlternateXLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("AlternateX | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with AlternateX specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					AlternateXLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("AlternateX | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with AlternateX specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					AlternateXLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= AlternateXLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("AlternateX | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with AlternateX specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						AlternateXLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("AlternateX | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with AlternateX specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					AlternateXLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			AlternateXLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			AlternateXLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('AlternateX had an issue creating settings.') end
...
function AlternateXLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function AlternateXLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function AlternateXLibrary:Destroy()
	AlternateXDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['AlternateX Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "AlternateXOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function AlternateXLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				AlternateXLibrary:Notify({Title = "AlternateX Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			AlternateXLibrary:Notify({Title = "AlternateX Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('AlternateX Configurations Error | '..tostring(result))
			AlternateXLibrary:Notify({Title = "AlternateX Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = AlternateXLibrary:CreateWindow({
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using AlternateX may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like AlternateX to get the key from
	...
	----AlternateXLibrary:Notify({Title = "AlternateX Interface", Content = "Welcome to AlternateX. These - are the brand new notification design for AlternateX, with custom sizing and AlternateX calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(AlternateXLibrary.Theme) do
...
task.delay(4, function()
	AlternateXLibrary.LoadConfiguration()
